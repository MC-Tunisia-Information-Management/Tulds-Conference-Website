    ...
    private function getSocialNetworkShareLink($net, $image) {
        ...
        switch ($net) {
            ...
            case 'twitter':
	            ...
	            $twitter_via_option_val = wanderland_mikado_options()->getOptionValue( 'twitter_via' );
	            $twitter_via            = '' !== $twitter_via_option_val ? esc_attr__( ' via ', 'masterds-core' ) . esc_attr( $twitter_via_option_val ) : '';
	            $link                   = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode( wanderland_mikado_the_excerpt_max_charlength( $count_char ) . $twitter_via ) . ' ' . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                break;
            case 'linkedin':
